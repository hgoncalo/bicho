  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        addToast({ type: 'success', message: `Bem-vindo de volta, ${data.username}!` })
        router.push('/dashboard')
      } else {
        addToast({ type: 'error', message: data.error || 'Credenciais inválidas.' })
      }
    } catch (err) {
      console.error(err)
      addToast({ type: 'error', message: 'Servidor indisponível. A acordar API...' })
    } finally {
      setLoading(false) 
    }
  }
          ...
          >
            {loading ? (
              <>
                ...
                A preparar a dashboard. Aguarde na página...
              </>
            ) : (
              'Entrar'
            )}
