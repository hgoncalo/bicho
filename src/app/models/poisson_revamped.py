...
def getAverages(home_df,away_df,real_df):
    ...
    avg['HomeFactor'] = ((avg['HomeAvgScored']/avg['AwayAvgScored'].replace(0,np.nan)).fillna(1)).clip(lower=0.9, upper=1.2)
...
def updateLambdaForces(current_round,game,lf,league_avg_scored,league_avg_against):
    ...
    updated_lf.loc[updated_lf['Team'] == home_team,'LambdaAtt'] = (((utils.POISSON_REVAMPED_EWMA * lambda_att_home) + ((1 - utils.POISSON_REVAMPED_EWMA) * old_lf_home['LambdaAtt'])) + (utils.POISSON_REVAMPED_LEARNING_BIAS * error_home))
    updated_lf.loc[updated_lf['Team'] == away_team,'LambdaAtt'] = (((utils.POISSON_REVAMPED_EWMA * lambda_att_away) + ((1 - utils.POISSON_REVAMPED_EWMA) * old_lf_away['LambdaAtt'])) + (utils.POISSON_REVAMPED_LEARNING_BIAS * error_away))
    updated_lf.loc[updated_lf['Team'] == home_team,'LambdaDef'] = (((utils.POISSON_REVAMPED_EWMA * lambda_def_home) + ((1 - utils.POISSON_REVAMPED_EWMA) * old_lf_home['LambdaDef'])) + (utils.POISSON_REVAMPED_LEARNING_BIAS * error_away))
    updated_lf.loc[updated_lf['Team'] == away_team,'LambdaDef'] = (((utils.POISSON_REVAMPED_EWMA * lambda_def_away) + ((1 - utils.POISSON_REVAMPED_EWMA) * old_lf_away['LambdaDef'])) + (utils.POISSON_REVAMPED_LEARNING_BIAS * error_home))
...
def predictGame(game,lf):
    ...
    lambda_home = lambda_home_att * lambda_away_def * home_factor * utils.POISSON_REVAMPED_OD
    lambda_away = lambda_away_att * lambda_home_def * (2 - away_factor) * utils.POISSON_REVAMPED_OD
...
def simulateCalendar(season,season_data,calendar,lf,averages):
    ...
    # Normalizar LFs
    mean_att = new_lf['LambdaAtt'].mean()
    mean_def = new_lf['LambdaDef'].mean()
    new_lf['LambdaAtt'] /= mean_att
    new_lf['LambdaDef'] /= mean_def
